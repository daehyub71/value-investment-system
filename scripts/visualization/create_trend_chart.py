...
"""
추세 지표 차트 시각화 시스템
GitHub DB 스키마 기반 완전 구현

실행 방법:
python scripts/visualization/create_trend_chart.py --stock_code=005930 --period=6M --output=html
python scripts/visualization/create_trend_chart.py --stock_code=000660 --save_png --show_signals
"""
        ...
        try:
            ...
            query = """
                SELECT 
                    ...
                    volume,
                    adjusted_close,
                    -- 기술적 지표들 (collect_stock_data.py에서 저장된 컬럼들)
                    sma_5,
                    sma_20,
                    sma_60,
                    sma_120,
            ...
            # 숫자 타입 변환
            numeric_columns = df.columns.drop(['date'] if 'date' in df.columns else [])
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='추세 지표 차트 생성')
    ...
    try:
        ...
        # 차트 생성
        print(f"📈 {args.stock_code} 추세 차트 생성 중...")
