#!/usr/bin/env python3
"""
DART API fs_div 오류 수정 버전
필수 파라미터 추가 및 API 호출 방식 개선

실행 예시:
python dart_collector_fs_div_fixed.py --year=2023 --companies=10
"""
...
class DartCollectorFsDivFixed:
    """fs_div 오류 수정된 DART 수집기"""
    ...
    def setup_logging(self):
        ...
        return logging.getLogger('DartCollectorFsDivFixed')
    ...
    def get_financial_statements_all_types(self, corp_code: str, stock_code: str, company_name: str, year: int) -> Optional[List[Dict]]:
        ...
        for fs_div in fs_div_types:
            try:
                ...
                if response.status_code != 200:
                    self.logger.warning(f"⚠️ HTTP 오류 {response.status_code}: {corp_code} ({fs_div})")
                    continue
                ...
                if data.get('status') == '000' and data.get('list'):
                    ...
                    self.logger.debug(f"✅ {fs_div}: {len(data['list'])}건 수집")
        ...
        if all_financial_data:
            self.logger.debug(f"✅ {company_name}: 총 {len(all_financial_data)}건 수집")
            return all_financial_data
        else:
            self.logger.debug(f"📭 모든 fs_div에서 데이터 없음: {company_name} ({year})")
            return None
    ...
    def collect_year_data(self, year: int, max_companies: Optional[int] = None) -> Dict[str, int]:
            ...
            try:
                ...
                if financial_data:
                    if self.save_financial_data(financial_data):
                        ...
                        self.logger.info(f"✅ {i:,}/{len(companies):,} {company_name}: {len(financial_data)}건")
                    else:
                        fail_count += 1
                        self.logger.warning(f"❌ 저장 실패: {company_name}")
                else:
                    fail_count += 1
                    self.logger.info(f"📭 데이터 없음: {company_name}")
        ...
        if success_count == 0:
            ...
            self.logger.info("   3. API 접근 권한 문제")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='fs_div 오류 수정된 DART 재무데이터 수집기')
    ...
    try:
        ...
        print(f"🚀 DART 재무데이터 수집기 시작 (fs_div 오류 수정)")
        print(f"📅 대상 연도: {', '.join(map(str, years))}")
        print(f"⏱️ API 간격: {args.delay}초")
        if args.companies:
            print(f"📊 수집 제한: {args.companies:,}개 종목")
        print("🔧 수정사항: fs_div 파라미터 자동 설정 (CFS, OFS)")
        print("=" * 70)
        
        collector = DartCollectorFsDivFixed(request_delay=args.delay)
